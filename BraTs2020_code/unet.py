...
class FFParser(nn.Module):
    def __init__(self, dim, h=128, w=65):
        ...
        self.complex_weight = nn.Parameter(torch.randn(
            dim, h, w, 2, dtype=torch.float32) * 0.02)
        self.w = w
        self.h = h
        ...
        if all_in_gpu:
            ...
            if use_gaussian and num_tiles > 1:
                ...
                add_for_nb_of_preds = torch.ones(
                    patch_size, device=self.get_device())
...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            if d < num_pool - 1:
                self.conv_trans_blocks_a.append(
                    conv_nd(2, int(d/2 + 1) * 128, 2 ** (d+5), 1))

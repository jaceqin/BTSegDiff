...
from .utils import staple, dice_score, norm, iou_score
...
class GaussianDiffusion:
    ...
    def sample_known(self, img, batch_size=1):
        ...
        return self.p_sample_loop_known(self.model, (batch_size, channels, image_size, image_size), img)
    ...
    def p_sample_loop_known(
        ...
        if self.dpm_solver:
            ...
            noise_schedule = NoiseScheduleVP(
                schedule='discrete', betas=th.from_numpy(self.betas))
            ...
            cal_out = torch.clamp(
                final["cal"] + 0.25 * final["sample"][:, -1, :, :].unsqueeze(1), 0, 1)
        else:
            print('no dpm-solver')
            i = 0
            ...
            for sample in self.p_sample_loop_progressive(
                ...
                all_model_output.append(sample["cal"].cpu())
                all_model_output_sample.append(sample["sample"].cpu())
            ######################
            if dice_score(final["sample"][:, -1, :, :].unsqueeze(1), final["cal"]) < 0.65:
                ...
                cal_out = torch.clamp(
                    final["cal"] * 0.5 + 0.5 * final["sample"][:, -1, :, :].unsqueeze(1), 0, 1)
            #######################
            final["all_samples"] = all_samples
            final["all_model_outputs"] = all_model_output
            final["all_model_outputs_sample"] = all_model_output_sample
        return final["sample"], x_noisy, img, final["cal"], cal_out, final
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            # terms["loss_dice"]= 1-dice_score(res,cal)
            if "vb" in terms:
